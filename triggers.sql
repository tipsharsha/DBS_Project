...
--Make a trigger to see that last post and new post created by the user are some time apart
-- CREATE OR REPLACE TRIGGER trg_time_between_posts
...
--         time_diff := EXTRACT(MINUTE FROM SYSTIMESTAMP - last_post_time);

--         IF time_diff < .5 THEN
--             RAISE_APPLICATION_ERROR(-20001, 'You can only create a new post after 30 seconds of your last post.');
--         END IF;
--     END IF;
-- END;
-- /
